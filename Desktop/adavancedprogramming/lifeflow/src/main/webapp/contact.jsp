...
<%
    ...
    if(currentUser != null) {
        if("admin".equals(currentUser.getRole())) {
            dashboardLink = request.getContextPath() + "/adminDashboard.jsp";
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        input[type="text"], input[type="email"], select, textarea { width: 100%; padding: 0.65rem 0.9rem; border: 1.5px solid var(--mid-gray); border-radius: var(--border-radius); font-size: 0.95rem; font-family: inherit; color: var(--dark-gray); background: #fdfdfd; transition: var(--transition); outline: none; }
    ...
    <div class="contact-layout">
        <aside aria-label="Contact information">
            <div class="contact-info">
                <article class="info-card">
                    ...
                    <div>
                        <h3>Our Address</h3>
                        <p>LifeFlow Blood Bank<br>Bagbazar<br>Kathmandu 44600<br>Nepal</p>
                    </div>
                </article>
                <article class="info-card">
                    <div class="card-icon">&#128222;</div>
                    <div>
                        ...
                        <p style="margin-top:6px;">Emergency Line (24/7):<br><a href="tel:014229999">01 422 9999</a></p>
                    </div>
                </article>
                <article class="info-card">
                    <div class="card-icon">&#128140;</div>
            ...
            <div class="map-placeholder" role="img" aria-label="Map location">
        ...
        <section aria-labelledby="formHeading">
            <div class="form-card">
                ...
                <form id="contactForm" action="ContactServlet" method="post" novalidate aria-label="Contact form">
                    <div class="form-grid">
                        <div class="form-group">
                            <label for="fullName">Full Name <span class="required" aria-label="required">*</span></label>
                            <input type="text" id="fullName" name="fullName" placeholder="e.g. Sita Thapa" maxlength="100" autocomplete="name" aria-required="true" value="<%= request.getParameter("fullName") != null ? request.getParameter("fullName") : "" %>">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" id="email" name="email" placeholder="e.g. sita@example.com" maxlength="150" autocomplete="email" aria-required="true" value="<%= request.getParameter("email") != null ? request.getParameter("email") : "" %>">
                        ...
                        <div class="form-group full-width">
                            ...
                            <textarea id="message" name="message" placeholder="Please describe your enquiry in detail..." maxlength="1000" aria-required="true"></textarea>
                            <span class="field-error" id="messageError">Please enter your message (minimum 10 characters).</span>
                        </div>
                        <div class="form-actions">
                            <button type="reset" class="btn btn-secondary" onclick="clearValidation()">Clear</button>
...
<script>
    ...
    function validateContactForm() {
        ...
        if (!emailRegex.test(email)) { showFieldError('email', 'emailError'); errors.push('A valid email address is required.'); isValid = false; }
        const subject = document.getElementById('subject').value;
        if (!subject) { showFieldError('subject', 'subjectError'); errors.push('Please select a subject.'); isValid = false; }
        const message = document.getElementById('message').value.trim();
        if (message.length < 10) { showFieldError('message', 'messageError'); errors.push('Message must be at least 10 characters.'); isValid = false; }
        if (!isValid) {
            const errorBox = document.getElementById('validationError');
            errorBox.innerHTML = '&#9888; Please fix the following issues:<ul style="margin-top:6px;padding-left:18px;">' + errors.map(e => `<li>${e}</li>`).join('') + '</ul>';
